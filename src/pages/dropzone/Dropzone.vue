<template>
  <form enctype='multipart/form-data' class="dropzone"
      @dragenter.prevent="hasDragged(true)"  @dragleave.prevent="hasDragged(false)"
      @dragover.prevent="hasDragged(true)" @drop.prevent="drop">

    <div :class="['dropzone_area', {'is-hover': isDragging} ]">
      <input type="file" name="images" id="images" required="required" multiple="multiple" @change="drop"/>
      <div>
        ...
        <div class="dropzone_title">DRAG & DROP your files</div>
        <div class="dropzone_text">to search for subtitles</div>
      </div>
    </div>
  </form>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      drop: function(e) {
        const files = (e.target || e.dataTransfer).files
        console.log('Looks you upload something!', JSON.stringify(files))
      },
